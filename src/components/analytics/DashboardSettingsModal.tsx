  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 w-full max-w-md translate-x-[-50%] translate-y-[-50%] bg-slate-900 text-slate-100 border border-slate-800 p-0 overflow-hidden shadow-2xl rounded-[24px] focus:outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]">
          <div className="px-6 pt-6 pb-4 relative">
            <Dialog.Title className="text-xl font-medium text-slate-100">Customize Dashboard</Dialog.Title>
            <Dialog.Close className="absolute right-6 top-6 text-slate-400 hover:text-slate-100 transition-colors">
              <X className="h-5 w-5" />
            </Dialog.Close>
          </div>
          ...
          <div className="px-6 pb-6 overflow-y-auto max-h-[60vh] custom-scrollbar">
            ...
            <div className="space-y-2">
              {widgetOptions.map((opt) => (
                <div
                  key={opt.id}
                  className="flex items-center justify-between p-4 rounded-[16px] bg-slate-800 border border-slate-700/50"
                >
                  <div>
                    <div className="text-[15px] font-medium text-slate-100">{opt.title}</div>
                    <div className="text-[13px] text-slate-400 mt-0.5">{opt.subtitle}</div>
                  </div>
                  <Switch
