-- ============================================================================
-- 0051: Timezone-aware analytics RPC
--
-- get_analytics_stats previously bucketed every daily/weekly/hourly
-- distribution in the database session timezone (UTC on Supabase), even though
-- each encounter stores the timezone it happened in. For any non-UTC user the
-- heatmap, hour-of-day, weekday and trend charts were shifted.
--
-- Adds an optional p_timezone parameter (defaults to UTC, validated against
-- pg_timezone_names) and buckets in the user's timezone.
-- ============================================================================

DROP FUNCTION IF EXISTS get_analytics_stats(UUID, UUID, TIMESTAMPTZ, TIMESTAMPTZ, BOOLEAN);

CREATE FUNCTION get_analytics_stats(
  p_user_id UUID,
  p_partner_id UUID DEFAULT NULL,
  p_start_date TIMESTAMPTZ DEFAULT NULL,
  p_end_date TIMESTAMPTZ DEFAULT NULL,
  p_climaxed BOOLEAN DEFAULT NULL,
  p_timezone TEXT DEFAULT NULL
)
RETURNS JSON AS $$
DECLARE
  v_partner_ids UUID[];
  v_tz TEXT := COALESCE(NULLIF(p_timezone, ''), 'UTC');
  v_now_local TIMESTAMP;
BEGIN
  -- An invalid zone would make every AT TIME ZONE call throw — fall back to UTC.
  IF NOT EXISTS (SELECT 1 FROM pg_timezone_names WHERE name = v_tz) THEN
    v_tz := 'UTC';
  END IF;
  v_now_local := now() AT TIME ZONE v_tz;

  v_partner_ids := resolve_partner_ids(p_user_id, p_partner_id);

  RETURN (
    WITH filtered AS (
      SELECT e.* FROM encounters e
      LEFT JOIN partners bp ON bp.id = e.partner_id AND bp.bound_user_id = p_user_id
      WHERE
        (v_partner_ids IS NULL
          AND e.partner_id IS NOT NULL
          AND (
            (e.user_id = p_user_id AND EXISTS (
              SELECT 1 FROM partners op
              WHERE op.id = e.partner_id
                AND op.user_id = p_user_id
                AND op.status = 'active'
            ))
            OR
            (bp.id IS NOT NULL AND EXISTS (
              SELECT 1 FROM partners op
              WHERE op.user_id = p_user_id
                AND op.source = 'bound'
                AND op.bound_user_id = bp.user_id
                AND op.status = 'active'
            ))
          ))
        OR (v_partner_ids IS NOT NULL AND e.partner_id = ANY(v_partner_ids))
    ),
    bounded AS (
      SELECT *, (started_at AT TIME ZONE v_tz) AS started_local
      FROM filtered
      WHERE (p_start_date IS NULL OR started_at >= p_start_date)
        AND (p_end_date IS NULL OR started_at < p_end_date)
        AND (p_climaxed IS NULL OR climaxed IS NOT DISTINCT FROM p_climaxed)
    ),
    stats AS (
      SELECT
        COUNT(*)::INT AS total_count,
        COALESCE(SUM(duration_minutes) FILTER (
          WHERE duration_minutes IS NOT NULL AND duration_minutes >= 0
        ), 0)::INT AS total_duration_sum,
        COUNT(*) FILTER (
          WHERE started_local >= date_trunc('week', v_now_local)
            AND started_local < date_trunc('week', v_now_local) + INTERVAL '7 days'
        )::INT AS week_count,
        COUNT(*) FILTER (
          WHERE started_local >= date_trunc('week', v_now_local) - INTERVAL '7 days'
            AND started_local < date_trunc('week', v_now_local)
        )::INT AS prev_week_count,
        COUNT(*) FILTER (
          WHERE started_local >= date_trunc('month', v_now_local)
            AND started_local < date_trunc('month', v_now_local) + INTERVAL '1 month'
        )::INT AS month_count,
        ROUND(AVG(duration_minutes) FILTER (WHERE duration_minutes IS NOT NULL AND duration_minutes >= 0))::INT AS avg_duration,
        ROUND((AVG(rating) FILTER (WHERE rating IS NOT NULL AND rating BETWEEN 1 AND 5))::NUMERIC, 1) AS avg_rating,
        MAX(started_at) AS last_encounter_at
      FROM bounded
    ),
    climaxed_counts AS (
      SELECT
        COUNT(*) FILTER (WHERE climaxed = TRUE)::INT AS climaxed_count,
        COUNT(*) FILTER (WHERE climaxed = FALSE)::INT AS non_climaxed_count
      FROM bounded
    ),
    geography AS (
      SELECT
        COUNT(DISTINCT LOWER(TRIM(e.city))) FILTER (WHERE e.city IS NOT NULL)::INT AS city_count,
        COUNT(DISTINCT e.country_code) FILTER (WHERE e.country_code IS NOT NULL)::INT AS country_count,
        COUNT(DISTINCT COALESCE(
          LOWER(TRIM(e.location_label)),
          CASE WHEN e.latitude IS NOT NULL AND e.longitude IS NOT NULL
            THEN ROUND(e.latitude::NUMERIC, 3)::TEXT || ',' || ROUND(e.longitude::NUMERIC, 3)::TEXT
            ELSE NULL
          END
        ))::INT AS footprint_count
      FROM bounded e
    ),
    daily_counts AS (
      SELECT
        to_char(date_trunc('day', started_local), 'MM-DD') AS label,
        COUNT(*)::INT AS value
      FROM bounded
      WHERE started_local >= v_now_local - INTERVAL '29 days'
      GROUP BY date_trunc('day', started_local)
      ORDER BY date_trunc('day', started_local)
    ),
    recent7_durations AS (
      SELECT COALESCE(duration_minutes, 0) AS d
      FROM bounded
      WHERE started_local >= v_now_local - INTERVAL '7 days'
        AND duration_minutes IS NOT NULL
        AND duration_minutes >= 0
      ORDER BY started_at DESC
      LIMIT 200
    ),
    weekly_trend AS (
      SELECT COALESCE(json_agg(json_build_object('label', label, 'value', value) ORDER BY label), '[]'::json) AS data
      FROM (
        SELECT to_char(w.week_start, 'MM/dd') AS label, COUNT(e.id)::INT AS value
        FROM (
          SELECT date_trunc('week', gs)::timestamp AS week_start
          FROM generate_series(
            date_trunc('week', v_now_local - INTERVAL '11 weeks'),
            date_trunc('week', v_now_local),
            '1 week'::interval
          ) gs
        ) w
        LEFT JOIN bounded e
          ON e.started_local >= w.week_start
          AND e.started_local < w.week_start + INTERVAL '7 days'
        GROUP BY w.week_start
        ORDER BY w.week_start
      ) sub
    ),
    monthly_trend AS (
      SELECT COALESCE(json_agg(json_build_object('label', label, 'value', value) ORDER BY label), '[]'::json) AS data
      FROM (
        SELECT to_char(m.month_start, 'YY-MM') AS label, COUNT(e.id)::INT AS value
        FROM (
          SELECT date_trunc('month', gs)::timestamp AS month_start
          FROM generate_series(
            date_trunc('month', v_now_local - INTERVAL '11 months'),
            date_trunc('month', v_now_local),
            '1 month'::interval
          ) gs
        ) m
        LEFT JOIN bounded e
          ON e.started_local >= m.month_start
          AND e.started_local < m.month_start + INTERVAL '1 month'
        GROUP BY m.month_start
        ORDER BY m.month_start
      ) sub
    ),
    duration_dist AS (
      SELECT COALESCE(json_agg(json_build_object('label', label, 'value', value) ORDER BY label), '[]'::json) AS data
      FROM (
        SELECT bucket AS label, COUNT(*)::INT AS value
        FROM (
          SELECT
            CASE
              WHEN duration_minutes < 15 THEN '0-15m'
              WHEN duration_minutes < 30 THEN '15-30m'
              WHEN duration_minutes < 45 THEN '30-45m'
              ELSE '45m+'
            END AS bucket
          FROM bounded
          WHERE duration_minutes IS NOT NULL
            AND duration_minutes >= 0
        ) t
        GROUP BY bucket
      ) sub
    ),
    weekday_dist AS (
      SELECT json_agg(json_build_object(
        'label', w.name,
        'value', COALESCE(c.value, 0)
      ) ORDER BY array_position(ARRAY['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], w.name)) AS data
      FROM (VALUES ('Mon', 1), ('Tue', 2), ('Wed', 3), ('Thu', 4), ('Fri', 5), ('Sat', 6), ('Sun', 0)) w(name, dow)
      LEFT JOIN (
        SELECT EXTRACT(DOW FROM started_local)::INT AS dow, COUNT(*)::INT AS value
        FROM bounded
        GROUP BY EXTRACT(DOW FROM started_local)
      ) c ON c.dow = w.dow
    ),
    timeofday_dist AS (
      SELECT json_agg(json_build_object(
        'label', p.label,
        'value', COALESCE(c.value, 0)
      ) ORDER BY array_position(ARRAY['Morning','Afternoon','Evening','Night'], p.label)) AS data
      FROM (VALUES ('Morning'), ('Afternoon'), ('Evening'), ('Night')) p(label)
      LEFT JOIN (
        SELECT
          CASE
            WHEN EXTRACT(HOUR FROM started_local) >= 6 AND EXTRACT(HOUR FROM started_local) < 12 THEN 'Morning'
            WHEN EXTRACT(HOUR FROM started_local) >= 12 AND EXTRACT(HOUR FROM started_local) < 18 THEN 'Afternoon'
            WHEN EXTRACT(HOUR FROM started_local) >= 18 AND EXTRACT(HOUR FROM started_local) < 24 THEN 'Evening'
            ELSE 'Night'
          END AS period,
          COUNT(*)::INT AS value
        FROM bounded
        GROUP BY period
      ) c ON c.period = p.label
    ),
    heatmap AS (
      SELECT json_agg(json_build_object(
        'date', to_char(ds.d, 'YYYY-MM-DD'),
        'count', COALESCE(d.value, 0)
      ) ORDER BY ds.d) AS data
      FROM (
        SELECT gs::date AS d
        FROM generate_series(
          (v_now_local::date - 364),
          v_now_local::date,
          '1 day'::interval
        ) gs
      ) ds
      LEFT JOIN (
        SELECT started_local::date AS d, COUNT(*)::INT AS value
        FROM bounded
        WHERE started_local >= (v_now_local::date - 364)
        GROUP BY started_local::date
      ) d ON d.d = ds.d
    ),
    tag_all AS (
      SELECT COALESCE(json_agg(json_build_object('label', name, 'value', value) ORDER BY value DESC, name), '[]'::json) AS data
      FROM (
        SELECT t.name, COUNT(*)::INT AS value
        FROM encounter_tags et
        JOIN bounded e ON e.id = et.encounter_id
        JOIN tags t ON t.id = et.tag_id
        GROUP BY t.id, t.name
        ORDER BY COUNT(*) DESC, t.name
        LIMIT 10
      ) sub
    ),
    tag_recent AS (
      SELECT COALESCE(json_agg(json_build_object('label', name, 'value', value) ORDER BY value DESC, name), '[]'::json) AS data
      FROM (
        SELECT t.name, COUNT(*)::INT AS value
        FROM encounter_tags et
        JOIN bounded e ON e.id = et.encounter_id
        JOIN tags t ON t.id = et.tag_id
        WHERE e.started_local >= v_now_local - INTERVAL '30 days'
        GROUP BY t.id, t.name
        ORDER BY COUNT(*) DESC, t.name
        LIMIT 6
      ) sub
    )
    SELECT json_build_object(
      'totalCount', stats.total_count,
      'totalDurationSum', stats.total_duration_sum,
      'weekCount', stats.week_count,
      'weekOverWeekChange', CASE
        WHEN stats.prev_week_count = 0 THEN CASE WHEN stats.week_count > 0 THEN 100 ELSE 0 END
        ELSE ROUND(((stats.week_count - stats.prev_week_count)::NUMERIC / stats.prev_week_count) * 100)
      END,
      'monthCount', stats.month_count,
      'avgDuration', stats.avg_duration,
      'avgRating', stats.avg_rating,
      'lastEncounterAt', stats.last_encounter_at,
      'cityCount', geography.city_count,
      'countryCount', geography.country_count,
      'footprintCount', geography.footprint_count,
      'climaxedCount', climaxed_counts.climaxed_count,
      'nonClimaxedCount', climaxed_counts.non_climaxed_count,
      'recent30Days', COALESCE(
        (SELECT json_agg(json_build_object('label', label, 'value', value) ORDER BY label) FROM daily_counts),
        '[]'::json
      ),
      'recent7DaysDurations', COALESCE(
        (SELECT json_agg(d ORDER BY d) FROM recent7_durations),
        '[]'::json
      ),
      'weeklyTrend12', weekly_trend.data,
      'monthlyTrend12', monthly_trend.data,
      'durationDistribution', duration_dist.data,
      'weekdayDistribution', weekday_dist.data,
      'timeOfDayDistribution', timeofday_dist.data,
      'heatmapData', heatmap.data,
      'tagRanking', tag_all.data,
      'topRecentTags', tag_recent.data
    )
    FROM stats, climaxed_counts, geography, weekly_trend, monthly_trend, duration_dist,
         weekday_dist, timeofday_dist, heatmap, tag_all, tag_recent
  );
END;
$$ LANGUAGE plpgsql STABLE;

COMMENT ON FUNCTION get_analytics_stats(UUID, UUID, TIMESTAMPTZ, TIMESTAMPTZ, BOOLEAN, TEXT) IS 'Unified analytics with date-range, climaxed filtering and user-timezone bucketing.';
