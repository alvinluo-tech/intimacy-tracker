-- Fix: Include bound partner encounters in analytics
-- The previous filtered CTE only matched user_id = p_user_id,
-- missing encounters created by bound partners under their own accounts.
...
BEGIN
  ...
  RETURN (
    WITH filtered AS (
      ...
      WHERE (v_partner_ids IS NULL AND (e.user_id = p_user_id OR bp.id IS NOT NULL))
    ...
    tag_all AS (
      ...
      FROM (
        ...
        WHERE ((v_partner_ids IS NULL AND (e.user_id = p_user_id OR EXISTS (
...
COMMENT ON FUNCTION get_analytics_stats(UUID, UUID, TIMESTAMPTZ, TIMESTAMPTZ) IS 'Unified analytics with date-range filtering and bound partner support.';
