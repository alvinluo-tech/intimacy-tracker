...
BEGIN
  ...
  WITH filtered AS (
    ...
    WHERE user_id = p_user_id
      AND (v_partner_ids IS NULL OR partner_id = ANY(v_partner_ids))
  ),
  stats AS (
    SELECT
...
BEGIN
  ...
  RETURN (
    ...
    LEFT JOIN encounters e
      ...
      AND e.started_at >= w.week_start
      AND e.started_at < w.week_start + INTERVAL '7 days'
      AND (v_partner_ids IS NULL OR e.partner_id = ANY(v_partner_ids))
...
BEGIN
  ...
  RETURN (
    ...
    LEFT JOIN encounters e
      ON e.user_id = p_user_id
      AND e.started_at >= m.month_start
