-- Add climaxed field to encounters table
-- Existing records predate this field and are known to have climaxed.
-- New records must send an explicit TRUE/FALSE value through the app.

ALTER TABLE encounters ADD COLUMN IF NOT EXISTS climaxed BOOLEAN DEFAULT NULL;

UPDATE encounters SET climaxed = TRUE WHERE climaxed IS NULL;

ALTER TABLE encounters ALTER COLUMN climaxed SET NOT NULL;

-- Update create_encounter_atomic to accept p_climaxed
DROP FUNCTION IF EXISTS create_encounter_atomic(UUID, UUID, TIMESTAMPTZ, TIMESTAMPTZ, NUMERIC, TEXT, BOOLEAN, TEXT, DOUBLE PRECISION, DOUBLE PRECISION, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, TEXT, JSONB, BOOLEAN, UUID[], TEXT[], BOOLEAN[]);
...
COMMENT ON FUNCTION create_encounter_atomic IS 'Creates an encounter with tags, photos, and climaxed flag in a single atomic transaction.';

-- Update get_analytics_stats to support climaxed filter and return climaxed counts
...
COMMENT ON FUNCTION get_analytics_stats(UUID, UUID, TIMESTAMPTZ, TIMESTAMPTZ, BOOLEAN) IS 'Unified analytics with date-range and climaxed filtering. Returns climaxedCount and nonClimaxedCount.';
