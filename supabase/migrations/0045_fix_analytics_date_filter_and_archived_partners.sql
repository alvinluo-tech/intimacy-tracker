-- Fix #10: Exclude archived partner encounters from "all partners" mode
-- Fix #4: Geography, heatmap, and tag charts now respect date range filter
-- Fix #3: Tag CTEs refactored to reuse bounded CTE instead of inline filter
...
BEGIN
  ...
  RETURN (
    WITH filtered AS (
      ...
      WHERE
        -- All partners: own active encounters + bound partner encounters (exclude archived partners)
        (v_partner_ids IS NULL
          AND e.partner_id IS NOT NULL
          AND (
            -- Own encounters with active (non-archived) partners
            ...
            OR
            -- Bound partner encounters where own partner is active
        ...
        -- Specific partner: encounters with that partner_id (including mirror)
...
COMMENT ON FUNCTION get_analytics_stats(UUID, UUID, TIMESTAMPTZ, TIMESTAMPTZ) IS 'Unified analytics with date-range filtering. Excludes unassigned and archived partner encounters. All charts respect date filter.';
