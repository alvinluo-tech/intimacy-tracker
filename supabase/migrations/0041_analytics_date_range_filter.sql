-- Phase 3: Date-range filtering for analytics RPC
-- Adds p_start_date / p_end_date parameters + totalDurationSum

DROP FUNCTION IF EXISTS get_analytics_stats(UUID, UUID);
...
BEGIN
  ...
  RETURN (
    ...
    -- Geography (always from full filtered set, not date-bounded)
    geography AS (
      SELECT
    ...
    -- Recent 30 days (always last 30 days from now, independent of date filter)
    recent30 AS (
    ...
    -- Duration distribution (from bounded range)
    duration_dist AS (
      SELECT COALESCE(json_agg(json_build_object('label', label, 'value', value) ORDER BY label), '[]'::json) AS data
      FROM (
    ...
    -- Weekday distribution (from bounded range)
    weekday_dist AS (
    ...
    -- Time of day distribution (from bounded range)
    timeofday_dist AS (
      SELECT json_agg(json_build_object(
        'label', p.label,
        'value', COALESCE(c.value, 0)
    ...
    -- Heatmap (365 days, always full year)
    heatmap AS (
      SELECT json_agg(json_build_object(
...
COMMENT ON FUNCTION get_analytics_stats(UUID, UUID, TIMESTAMPTZ, TIMESTAMPTZ) IS 'Unified analytics with optional date-range filtering. Pass p_start_date/p_end_date to bound card stats, charts, and distributions.';
